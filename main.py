...
class Config:
    def __init__(self, dataset, meta_train_iterations, meta_lr, inner_lr,
                 meta_batch_size, img_size, num_channels, k_shot, n_way,
                 data_path, num_inner_updates, output_dir, load_model_path):
...
def save_metric_results(current_run_savedir, metrics):
...
def meta_train(model, data_generator, meta_train_iterations, config):
    ...
    custom_savename = f"{config.dataset}_K_{config.k_shot}_N_{config.n_way}_mlr_{config.meta_lr}_ilr_{config.inner_lr}_{timestamp}"
    current_run_savedir = create_output_directory(config.output_dir, custom_savename)
    ...
    for meta_step in range(meta_train_iterations):
        ...
        for task_index in range(config.meta_batch_size):
            ...
            for inner_loop_num in range(config.num_inner_updates):
                ...
                support_output = model.forward(support_input, adapted_weights,
                                               cuda=False) if inner_loop_num != 0 else model.forward(support_input,
                                                                                                     cuda=False)
        ...
        if meta_step % save_interval == 0:
            ...
            save_metric_results(current_run_savedir, metrics_snapshot)
            # Save model
            save_path = Path(config.output_dir) / custom_savename / "models" / f"model_epoch_{meta_step}"
            print(f"Saving model at path {save_path}")
            torch.save(model.state_dict(), str(save_path))
            # Remove outdated model saves
            old_model_path = Path(config.output_dir) / custom_savename / "models" / f"model_epoch_{meta_step - 3 * save_interval}"
...
def main(dataset, meta_train_iterations, meta_lr, inner_lr, meta_batch_size, img_size, num_channels, k_shot, n_way, data_path,
         num_inner_updates, output_dir, load_model_path):
    # Pack configurable parameters into single object
    config = Config(dataset, meta_train_iterations, meta_lr, inner_lr, meta_batch_size, img_size, num_channels, k_shot, n_way, data_path,
         num_inner_updates, output_dir, load_model_path)
    #tracked_meta_train_losses, tracked_meta_train_accuracies, tracked_meta_val_losses, tracked_meta_val_accuracies = [], [], [], []
    ...
    meta_trained_model = meta_train(model, data_generator, meta_train_iterations, config)
